...
## Create `.env.local` file in `chaktech-storefront/` directory
...
The ChakTech backend mounts its routes directly at the API root.

Examples:

- `GET ${NEXT_PUBLIC_API_BASE_URL}/pages/:slug/:locale`
- `GET ${NEXT_PUBLIC_API_BASE_URL}/content-blocks/:page/:locale`
- `GET ${NEXT_PUBLIC_API_BASE_URL}/menus/:position/:locale`

Do **not** prefix these backend routes with `/api/` in storefront fetch calls. The storefront should call `/pages/...` and `/content-blocks/...` directly against `NEXT_PUBLIC_API_BASE_URL`.
